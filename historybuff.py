...
def get_first_event(day):
    try:
        ...
        outp = "Is that a real day? I'm not sure. Can you try again? For which day would you like events for?"
...
@ask.intent('CanFulfillIntentRequest', convert={ 'day': 'date' })
def get_first_event2(day):
    month_name = day.strftime('%B')
    day_number = day.day
    events = _get_json_events_from_wikipedia(month_name, day_number)
    if not events:
...
def stop():
    outp = "Sorry I didn't get that. For which day would you like events for?"
    return question(outp).reprompt(outp)
